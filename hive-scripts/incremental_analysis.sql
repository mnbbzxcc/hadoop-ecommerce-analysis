...
-- 每日统计（覆盖指定日期的数据）
INSERT OVERWRITE TABLE daily_stats PARTITION (dt='${hivevar:dt}')
SELECT 
    '${hivevar:dt}' AS dt,
    COUNT(*) AS pv,
...
-- 每小时统计（覆盖指定日期）—— 直接使用 UTC 小时
INSERT OVERWRITE TABLE hourly_stats PARTITION (dt='${hivevar:dt}')
SELECT 
    '${hivevar:dt}' AS dt,
...
-- 热门商品 Top 10（覆盖指定日期）
INSERT OVERWRITE TABLE top_products PARTITION (dt='${hivevar:dt}')
SELECT 
    product_id,
    COUNT(*) AS pv,
    ...
    '${hivevar:dt}' AS dt
FROM user_behavior
WHERE behavior='pv' AND dt = '${hivevar:dt}'
GROUP BY product_id
ORDER BY pv DESC
LIMIT 10;

-- 热门类目 Top 10（覆盖指定日期）
INSERT OVERWRITE TABLE top_categories PARTITION (dt='${hivevar:dt}')
SELECT 
    category_id,
    COUNT(*) AS pv,
...
-- 转化漏斗（覆盖指定日期）
INSERT OVERWRITE TABLE funnel_stats PARTITION (dt='${hivevar:dt}')
SELECT 
    '${hivevar:dt}' AS dt,
